...
// True once the deferred fill for the CURRENT frame's group 0 (active_line 0)
// has been serviced. Set false at the vsync line when video_output_arm_group0_prefill()
// queues the next frame's group-0 fill; set true by
// video_output_service_double_buffer_fill() right after it renders active_line 0.
// video_output_prefill_group0() uses it to skip the synchronous ISR fill when
// the vblank-pipelined fill already ran (see the comment on
// video_output_arm_group0_prefill()).
static volatile bool db_group0_filled;
...
// Pipeline the NEXT frame's group 0 (active_line 0) at the vsync line
// (v_scanline==rt_v_front_porch) instead of filling it synchronously at the
// last back-porch line. By vsync, the previous frame's final DATA segment has
// completed, so db_buffers[0] is free, and the deferred fill (Core 1
// background loop / db_fill IRQ) renders it with the ENTIRE vblank remainder
// as its budget. That removes the last scanline-callback run still bound to
// the single-segment ISR budget: an expensive 720P blur fill (~12k cycles at
// 372 MHz) overran it at every frame start, the HSTX FIFO underflowed, and
// the sink dropped the signal the moment blur was enabled. video_output_prefill_group0()
// at the back-porch line remains as the fallback for the case where this
// deferred fill was never serviced (e.g. first frame after a mode switch).
// Deliberately NOT __scratch_x: runs once per frame, and keeping the body in
// regular RAM preserves scratch_x headroom for the hot per-line path. The
// active_data/arm-fire handshake is deliberately NOT used here: it relies on
// the next active-data call to fire the request, which never happens during
// vblank -- by the time this runs, the fill target is already safe to touch.
static void video_output_arm_group0_prefill(void)
{
    if (!rt_double_buffer_active) {
        return;
    }
    db_front_idx = 0;
    db_group0_filled = false;
    db_fill_dst = db_buffers[0];
    db_fill_v_scanline = rt_blank_head;
    db_fill_active_line = 0;
...
// Since the vsync-time deferred fill (video_output_arm_group0_prefill())
// normally pipelined group 0 already during the vblank, this is usually a
// cheap early return; the synchronous fill is the fallback for a missed
// deferred fill (e.g. first frame after a mode switch).
...
static void __scratch_x("") video_output_prefill_group0(void)
{
    db_front_idx = 0;
    if (db_group0_filled) {
        // The vblank-pipelined fill already rendered line 0 into db_buffers[0].
        return;
    }
    db_fill_pending = false;
...
static void video_output_service_double_buffer_fill(void)
{
    ...
    // The only request that ever targets active_line 0 is the vsync-pipelined
    // group-0 fill; record that it ran so the back-porch ISR skips its own
    // synchronous prefill (see video_output_prefill_group0()).
    if (al == 0U) {
        __compiler_memory_barrier();
        db_group0_filled = true;
    }
}
...
static void init_rt_from_mode(const video_mode_t *mode)
{
    ...
    // No frame has had its group-0 fill serviced yet under this mode; the
    // next vsync arms it (see video_output_arm_group0_prefill()).
    db_group0_filled = false;
#endif
}
