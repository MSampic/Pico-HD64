...
void config_load(void)
{
    ...
    if (tmp.magic1 == CONFIG_MAGIC1 && tmp.magic2 == CONFIG_MAGIC2 &&
        ...
        // Single-mode build: the output mode is fixed at COMPILE time (see
        // N64_HDMI_OUTPUT_* in CMakeLists / main.c's boot_mode_sel), so the
        // saved output_mode is forced back to this build's mode. Everything
        // else (zoom/pos/blur/aspect/sample rate) still comes from the saved
        // config. This is also what makes a saved config written by the
        // 3-mode overlay app safe to boot here: its output_mode (e.g. 720P)
        // is overridden, but the picture settings carry over.
        g_config.output_mode = default_config.output_mode;
#else
