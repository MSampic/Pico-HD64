...
/**
 * @brief Report whether any joybus data has been received since init.
 * @return true once at least one word has been received from the PIO.
 *
 * The N64 only polls the controller once the game has called osContInit()
 * (during the IPL the PIF is busy with the cartridge CIC handshake), so
 * there is a window after boot where the receiver has seen nothing. This
 * tells the caller whether a real poll has arrived yet - which a raw
 * reading cannot, since a poll with all buttons released also reads 0.
 */
