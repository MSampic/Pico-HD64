...
// ----------------------------------------------------------------------------
#if !defined(N64_HDMI_SINGLE_MODE)
...
#if defined(N64_HDMI_SINGLE_MODE)
    // Single-mode build (apps/n64_hdmi): the output mode is fixed at
    // COMPILE time (the N64_HDMI_OUTPUT_* define from CMakeLists), so
    // boot_mode_sel is a constant here and the switch below always takes
    // the same path. The OSD has no "Video Mode" item and config_load()
    // forces output_mode back to this mode, so nothing can change it at
    // runtime. The clock/voltage/descriptor cases for the other modes stay
    // compiled but are unreachable (all three descriptors are always
    // available, so this costs no extra image size beyond dead branches).
#if defined(N64_HDMI_OUTPUT_1280x720)
    const uint32_t boot_mode_sel = 2;
#elif defined(N64_HDMI_OUTPUT_640x480)
    const uint32_t boot_mode_sel = 1;
#else
    const uint32_t boot_mode_sel = 0;
#endif
#else
