 ...
 *
 * Regardless of which mode the build defaults to, the 3-mode overlay app
 * (apps/n64_hdmi_osd_overlay) makes all three modes available at runtime:
 * the OSD's "Video Mode" item stages output_mode and "Save & Reboot"
 * persists it, and main() picks the matching sys_clk / descriptor at boot.
 *
 * This app (apps/n64_hdmi, N64_HDMI_SINGLE_MODE) instead compiles to exactly
 * ONE mode: the OSD has no "Video Mode" item, and config_load() forces
 * output_mode back to this build's mode (the N64_HDMI_OUTPUT_* define), so a
 * saved config - even one written by the overlay app - can never change the
 * output mode here. Only the non-mode settings (zoom/pos/blur/aspect) carry
 * over.
 */
...
typedef struct config {
    ...
    uint32_t output_mode;           ///< 0 = 240P, 1 = 480P, 2 = 720P. 3-mode overlay app: staged in the OSD and applied by "Save & Reboot" at boot (which also picks the sys_clk). Single-mode build (N64_HDMI_SINGLE_MODE): fixed at compile time and forced back on every config_load(); the field is kept so the config layout (and saved settings) stay interchangeable with the overlay app.
